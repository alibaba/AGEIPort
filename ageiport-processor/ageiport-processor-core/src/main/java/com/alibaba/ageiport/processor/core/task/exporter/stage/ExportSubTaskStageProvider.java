...
public class ExportSubTaskStageProvider implements SubTaskStageProvider {
    ...
    public static Stage S04_EXECUTE_QUERY_DATA_START = new StageImpl(GROUP, 400, "S04_EXECUTE_QUERY_DATA_START", "查询数据-开始", 0.21, 0.21, "");


    public static Stage S05_EXECUTE_QUERY_DATA_END = new StageImpl(GROUP, 500, "S05_EXECUTE_QUERY_DATA_END", "查询数据-结束", 0.70, 0.70, "");

    public static Stage S06_EXECUTE_CONVERT_START = new StageImpl(GROUP, 600, "S06_EXECUTE_CONVERT_START", "转换数据-开始", 0.71, 0.71, "");

    public static Stage S07_EXECUTE_CONVERT_END = new StageImpl(GROUP, 700, "S07_EXECUTE_CONVERT_END", "转换数据-结束", 0.85, 0.85, "");

    public static Stage S08_EXECUTE_GROUP_START = new StageImpl(GROUP, 800, "S08_EXECUTE_GROUP_START", "视图分组-开始", 0.87, 0.87, "");

    public static Stage S09_EXECUTE_GROUP_END = new StageImpl(GROUP, 900, "S09_EXECUTE_GROUP_END", "视图分组-结束", 0.89, 0.89, "");

    public static Stage S10_EXECUTE_TO_MAP_START = new StageImpl(GROUP, 1000, "S10_EXECUTE_TO_MAP_START", "转换为DataSet-开始", 0.90, 0.90, "");

    public static Stage S11_EXECUTE_TO_MAP_END = new StageImpl(GROUP, 1100, "S11_EXECUTE_TO_MAP_END", "转换为DataSet-结束", 0.91, 0.91, "");

    public static Stage S12_SAVE_DATA_START = new StageImpl(GROUP, 1200, "S12_SAVE_DATA_START", "存储数据-开始", 0.92, 0.92, "");


    public static Stage S13_SAVE_DATA_END = new StageImpl(GROUP, 1300, "S13_SAVE_DATA_END", "存储数据-结束", 0.99, 0.99, "");


    public static Stage S14_FINISHED = new StageImpl(GROUP, 1400, CommonStage.FINISHED.getCode(), "子任务已完成", 1.00, 1.00, "", true, true, false);


    public static Stage S14_ERROR = new StageImpl(GROUP, 1400, CommonStage.ERROR.getCode(), "子任务失败", 1.00, 1.00, "", true, false, true);
