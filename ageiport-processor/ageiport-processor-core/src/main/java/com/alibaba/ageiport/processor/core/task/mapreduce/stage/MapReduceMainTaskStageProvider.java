 ...
 * @author lingyi
 */
public class MapReduceMainTaskStageProvider implements MainTaskStageProvider {
    ...
    Stage S09_GET_HEADERS_START = new StageImpl(GROUP, 900, S09_GET_HEADERS_START_CODE, "获取表头-开始", 0.11, 0.11, "");

    Stage S10_GET_HEADERS_END = new StageImpl(GROUP, 1000, S10_GET_HEADERS_END_CODE, "获取表头-结束", 0.12, 0.12, "");

    Stage S11_GET_DYNAMIC_HEADERS_START = new StageImpl(GROUP, 1100, S11_GET_DYNAMIC_HEADERS_START_CODE, "获取动态表头-开始", 0.13, 0.13, "");

    Stage S12_GET_DYNAMIC_HEADERS_END = new StageImpl(GROUP, 1200, S12_GET_DYNAMIC_HEADERS_END_CODE, "获取动态表头-结束", 0.14, 0.14, "");

    Stage S13_READ_FILE_START = new StageImpl(GROUP, 1300, S13_READ_FILE_START_CODE, "读取文件数据-开始", 0.09, 0.09, "");

    Stage S14_READ_FILE_END = new StageImpl(GROUP, 1400, S14_READ_FILE_END_CODE, "读取文件数据-结束", 0.10, 0.10, "");
    ...
    public MapReduceMainTaskStageProvider() {
